...
[] Ensure Level 1 starts just with simple flat blocks (the first of each color in the sprite sheet), with later levels generating the blocks with patterns on them (like the triangle, cross, etc.). These should be worth more points, at your discretion.
    Bugs: 
    - [FIXED but not happy] new tiles spawn of random variants
    TBI: 
    - tile variants count for more points (under score)
    - do tile variants need to match among themselves or only count more then? 
[] Create random shiny versions of blocks that will destroy an entire row on match, granting points for each block in the row.
    TBI: 
    - improve shiny look
    - implement for vertical matches 
    - implement for last of row / col matches 
    - play a sound and maybe shake screen?? (https://love2d.org/forums/viewtopic.php?t=81637)
    Bug: 
    - Does not spawn as extra tiles (just on base)
